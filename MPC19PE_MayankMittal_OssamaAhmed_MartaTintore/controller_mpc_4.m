...
function [param, yalmip_optimizer] = init()
    ...
    for k = 1:N-1
       ...
       % objective term: slackness variable
       objective = objective + Eps{:, k}' * S * Eps{:, k} + v * max(Eps{:, k});
       ...
       constraints = [constraints, G_x * X{:, k} <= h_x + repmat(Eps{:, k}, 2, 1)];
       if k > 1
    ...
    % since we want the terminal state to lie in X_f, no slackness present
    constraints = [constraints, Eps{:, N} == 0];
